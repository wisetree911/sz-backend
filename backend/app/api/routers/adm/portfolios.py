...
async def get_by_id(portfolio_id: int, service: PortfolioService=Depends(get_porfolio_service)) -> PortfolioResponseAdm:
    return await service.get_portfolio_by_portfolio_id(portfolio_id=portfolio_id)

@router.get("/")
async def get_all(service: PortfolioService=Depends(get_porfolio_service)) -> list[PortfolioResponseAdm]:
    return await service.get_all_portfolios()

@router.post("/")
async def create(payload: PortfolioCreateAdm, service: PortfolioService = Depends(get_porfolio_service)) -> PortfolioResponseAdm:
...
async def update_portfolio(portfolio_id: int, payload: PortfolioUpdateAdm, service: PortfolioService=Depends(get_porfolio_service)) -> PortfolioResponseAdm:
